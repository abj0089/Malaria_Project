...
all_data %>%
  mutate(RD = Deep_breathing) %>%
  group_by(Study_ID, Study_Number) %>%
  summarise(N = sum(RD, na.rm = T),
            PY = unique(Total_Population)) %>%
  mutate(N = ifelse(is.na(N), 0, N)) -> case_numbers

all_data %>%
  mutate(RD = Deep_breathing) %>%
  filter(!(is.na(Deep_breathing) & is.na(Inter_costal_recession))) %>% 
  mutate(Prob_class = ifelse(!Inter_costal_recession | is.na(Inter_costal_recession), 1, 2)) %>%
  dplyr::select(Study_Number, RD, Prob_class) -> statuses_pre

statuses_pre %>%
  mutate(NA_stat = is.na(RD)) %>% 
  filter(NA_stat, ) %>%
...
not_diagnosed %>%
  ...
  dplyr::select(-4)-> to_add

statuses_pre %>%
  filter(!is.na(RD)) %>%
  group_by(Study_Number, Prob_class) %>%
  summarise(N_positives = sum(RD), N_patients = n()) %>%
  ungroup() %>%
...
# FOR GLM #
###########
vars <- c('PR_corrected', 'PR', 'P_prime', 'rho', 'intercept', 'beta1', 'beta2', 'beta3', 'estimated_admissions', 'r')
...
jags_data <- list(
  ...
  N_NA = as.matrix(dplyr::select(to_add, 2:3)), 
...
save(all_fits, file = '../../Output/PfPR_model_fits_RD.RData')
...
save(all_DIC, file = '../../Output/PfPR_model_DIC_RD.RData')

c(1346, 835.8, 820.8, 826.1) - min(c(1346, 835.8, 820.8, 826.1))
